...
//            disableFields(paymentBox);
        ...
        submitButton.setOnAction(e -> {
            ...
            {
                popupStage.close();
                eingabeFehler("Alle Eingaben müssen gestellt werden");
                return;
            }
            ...
            if (selectedOption == payment) {
                if (amount.getText().indexOf("-") != amount.getText().lastIndexOf("-") || amount.getText().indexOf("-") != 0) {
                    popupStage.close();
                    eingabeFehler("Falsch geschribenes '-' Zeichen");
                    return;
                }
                try {
    ...
    private void eingabeFehler(String nachricht){
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
